...
import com.skillbox.a14homework.SingleLiveEvent

// Класс отвечает за хранения данных
class DataViewModel : ViewModel() {

    //    Поле хранит класс (репозиторий), который производит все изменения с данными
    ...
    //    Первоначальный список объектов
    private val celestialBodiesLiveData =
        MutableLiveData<List<CelestialBodies>>(
            listOf(
    ...
    //    Метод добавления нового элемента. Спиок использует метод, который отслеживает изменения (postValue).
    //    В качестве аргумента принимается метод репозитория, который добавляет элемент и возвращает новый список
    fun addNewElement(item: CelestialBodies) {
        celestialBodiesLiveData.postValue(
            repository.newElement(
    ...
    //    Метод удаления элемента. Спиок использует метод, который отслеживает изменения (postValue).
    //    В качестве аргумента принимается метод репозитория, который удаляет элемент и возвращает новый список.
    //    После удаления 1 раз показывается тост
